...
export interface ChartProps<T extends ChartDataPoint = ChartDataPoint> {
  data: T[];
}
...
const tooltipStyle: React.CSSProperties = { background: '#220910', border: '1px solid rgba(233,8,44,.18)', borderRadius: 12, color: '#fff' };

export function DemandChart({ data }: ChartProps) {
  return (
...
export function DistributionChart({ data }: ChartProps<DistributionDataPoint>) {
  return (
    <ResponsiveContainer width="100%" height={280}>
      <PieChart>
...
export function TransformerChart({ data }: ChartProps) {
  return (
