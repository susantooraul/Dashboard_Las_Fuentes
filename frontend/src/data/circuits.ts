...
export interface Circuit extends CircuitBase {
  kw: number;
  kwh: number;
  capacityKw: number;
...
export interface CircuitGroupBase {
  key: string;
  title: string;
  summary: string;
  tone: string;
  circuits: CircuitBase[];
}

export interface CircuitGroup extends Omit<CircuitGroupBase, 'circuits'> {
  circuits: Circuit[];
}

const BASE_GROUPS: CircuitGroupBase[] = [
  {
    key: 'linea1',
    title: 'Línea 1',
...
function enrichCircuit(item: CircuitBase, groupIndex: number, circuitIndex: number): Circuit {
...
const PLANT_GROUPS: CircuitGroup[] = BASE_GROUPS.map((group, groupIndex) => ({
  ...group,
...
export function getCircuitGroupsForSection(section: string): CircuitGroup[] {
  const keys = SECTION_ALIASES[section] ?? [];
  return PLANT_GROUPS.filter((group) => keys.includes(group.key));
}

export function getCircuitsForTransformer(transformerKey: string): CircuitGroup[] {
  return PLANT_GROUPS
...
export function getLineGroups(): CircuitGroup[] {
