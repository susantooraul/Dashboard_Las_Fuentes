export type PlantStatus = 'normal' | 'atencion' | 'critico';
...
export interface PlantStatusMeta {
  statusLabel: string;
...
export interface MultiPlantMockItem extends PlantCatalogItem, PlantStatusMeta {
  id: string;
  currentLoad: number;
  peakLoad: number;
  averageLoad: number;
...
const statusCycle: PlantStatus[] = ['normal', 'atencion', 'critico'];

const PLANT_CATALOG: PlantCatalogItem[] = [
...
function buildHistory(index: number, baseLoad: number): PlantHistoryPoint[] {
...
function getStatusMeta(status: PlantStatus): PlantStatusMeta {
  if (status === 'critico') {
    return {
      statusLabel: 'Crítico',
...
export function getMultiPlantMock(): MultiPlantMockItem[] {
