...
import { getLineGroups, type CircuitGroup } from '../data/circuits';
...
interface LineGroupSummary extends CircuitGroup {
  totalKw: number;
  capacityKw: number;
...
function LineSummaryCard({ group, index }: LineSummaryCardProps) {
  return (
...
export default function LinesOverviewPage({ setHeaderMeta }: LinesOverviewPageProps) {
  const lineGroups: LineGroupSummary[] = getLineGroups().map((group) => {
