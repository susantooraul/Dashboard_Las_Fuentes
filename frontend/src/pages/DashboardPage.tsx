...
import { defaultAlertSettings, getAlertSettings, saveAlertSettings, type AlertSettings } from '../services/alertService';
...
type DateFilterKey = 'startDate' | 'endDate' | 'preset';
...
interface HeaderMeta {
  title: string;
  subtitle: string;
  onExport: (format: HeaderExportFormat) => void | Promise<void>;
...
interface DashboardTableRow extends DataTableRow {
...
interface DashboardPayload {
  ...
  systems_data: DashboardSystemData[];
  table_data: DashboardTableRow[];
}
...
interface CircuitSchedule {
  startTime: string;
  stopTime: string;
  scheduleLabel: string;
}

interface FlattenedCircuit extends Omit<Circuit, 'amps'> {
  groupKey: string;
  groupTitle: string;
  fullLabel: string;
  displayName: string;
...
interface TransformerChartPoint {
  hour: string;
  vAB: number;
  vBC: number;
  vCA: number;
...
interface SubstationSeriesPoint extends Omit<TransformerChartPoint, 'kva'> {
  demand: number;
}

interface MonthlyConsumptionPoint {
  month: string;
  value: number;
}
...
interface SubstationCharts {
  series: SubstationSeriesPoint[];
...
function getQuickRange(key: string, referenceDate = new Date()): DateFilters {
...
function buildStableDemoSchedule(item: Partial<Circuit>, groupTitle?: string): CircuitSchedule {
...
function flattenCircuits(groups: CircuitGroup[]): FlattenedCircuit[] {
...
function normalizeCircuitKey(value: unknown): string {
  return String(value || '')
    .normalize('NFD')
    .replace(/[̀-ͯ]/g, '')
    .toLowerCase()
...
function buildTransformerCircuitChartData(circuits: FlattenedCircuit[], systemsData: DashboardSystemData[] = []): FlattenedCircuit[] {
...
function evaluateAlerts(payload: DashboardPayload | null, settings: AlertSettings): DashboardAlert[] {
  if (!payload) return [];
...
function HeroPanel({ title, subtitle, updatedAt, blue = false }: { title: string; subtitle: string; updatedAt: string | number | Date; blue?: boolean }) {
  return (
...
function AlertPopup({ alert, onClose }: { alert: DashboardAlert | null; onClose: () => void }) {
...
function DateFilterBar({ values, draft, onDraftChange, onApply, onReset, disabled = false }: { values: DateFilters; draft: DateFilters; onDraftChange: (key: DateFilterKey, value: string) => void; onApply: () => void; onReset: () => void; disabled?: boolean }) {
  return (
...
function AlertSettingsPage({ payload, settings, onSave, alerts }: { payload: DashboardPayload | null; settings: AlertSettings; onSave: (settings: AlertSettings) => void; alerts: DashboardAlert[] }) {
  const [form, setForm] = useState(settings);
  useEffect(() => setForm(settings), [settings]);

  const update = (key: keyof AlertSettings, value: number | boolean) => setForm((current) => ({ ...current, [key]: value }));
...
function CircuitGroupCard({ group }: { group: CircuitGroup }) {
  return (
...
function CircuitGrid({ groups, title, subtitle }: { groups: CircuitGroup[]; title: string; subtitle: string }) {
...
function CircuitMetricChart({ data, title, subtitle, metricKey, metricLabel, colors }: { data: FlattenedCircuit[]; title: string; subtitle: string; metricKey: 'kw' | 'kwh'; metricLabel: string; colors: string[] }) {
...
function DistributionPanel({ section, payload, sectionGroups }: { section: string; payload: DashboardPayload; sectionGroups: CircuitGroup[] }) {
...
function buildTransformerCharts(rows: DashboardTableRow[]): TransformerChartPoint[] {
  return rows.map((row) => {
...
function buildSubstationCharts(rows: DashboardTableRow[]): SubstationCharts {
...
function OperationSchedulePanel({ circuits, title, subtitle }: { circuits: FlattenedCircuit[]; title: string; subtitle: string }) {
...
function CircuitSimpleList({ circuits, title, subtitle }: { circuits: FlattenedCircuit[]; title: string; subtitle: string }) {
  if (!circuits.length) return null;
  return (
    <section className="panel panel-spacious fade-up">
      <div className="panel-header">
...
function SubstationView({ payload, rangeText }: { payload: DashboardPayload; rangeText: string }) {
...
function TransformerDetailView({ payload, section, rangeText }: { payload: DashboardPayload; section: string; rangeText: string }) {
...
function GeneralView({ payload, section }: { payload: DashboardPayload; section: string }) {
...
export default function DashboardPage({ plantId, section, setHeaderMeta }: DashboardPageProps) {
  const [payload, setPayload] = useState<DashboardPayload | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState('');
  const [message, setMessage] = useState<string | undefined>('');
  const [referenceDate, setReferenceDate] = useState(() => new Date());
  const [filters, setFilters] = useState<DateFilters>(() => getQuickRange(DEFAULT_FILTER_PRESET));
  const [draftFilters, setDraftFilters] = useState<DateFilters>(() => getQuickRange(DEFAULT_FILTER_PRESET));
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        setLoading(true);
        const data = await fetchDashboard(plantId, section, filters) as DashboardPayload;
        setPayload(data);
        setHeaderMeta({
          title: data.title,
          ...
          onEmail: async () => {
            const response = await sendReportByEmail({ to: 'ia@rsrc.com.mx', plant_id: plantId, section, format: 'pdf', subject: `Reporte ${data.title}`, message: `Adjunto reporte automático de ${data.title}` }) as EmailReportResponse;
  ...
  const handleSaveAlerts = (next: AlertSettings) => {
  ...
  const handleDraftChange = (key: DateFilterKey, value: string) => {
    if (key === 'preset') {
      if (!value) {
