...
import PlantDashboardCard, { type PlantDashboardData } from '../components/PlantDashboardCard';
import PlantMultiSelector from '../components/PlantMultiSelector';
import PlantsComparisonChart, { type PlantsComparisonItem } from '../components/PlantsComparisonChart';
import { getMultiPlantMock, type MultiPlantMockItem } from '../data/multiPlantMock';

type MultiPlantViewItem = MultiPlantMockItem & PlantDashboardData & PlantsComparisonItem;

const mockPlants = getMultiPlantMock() as unknown as MultiPlantViewItem[];
...
interface MultiPlantDashboardPageProps {
  setHeaderMeta: (meta: HeaderMeta) => void;
}

interface MultiPlantSummary {
...
export default function MultiPlantDashboardPage({ setHeaderMeta }: MultiPlantDashboardPageProps) {
  const [selectedPlantIds, setSelectedPlantIds] = useState<string[]>(mockPlants.map((plant) => plant.id));
  ...
  const visiblePlants: MultiPlantViewItem[] = useMemo(
    () => mockPlants.filter((plant) => selectedPlantIds.includes(plant.id)),
    [selectedPlantIds],
  );

  const summary: MultiPlantSummary = useMemo(() => {
