...
const USER_ACTIVITY_WINDOW_MS = 30000;
...
function writeSharedValue(key, value) {
    ...
    catch { /* memoria mantiene la sesión de esta vista */ }
}
...
function isPrivateOrLocalHost(host) {
    if (BOS_LOCAL_HTTP_HOSTS.has(host))
        return true;
...
function readBosLocalSessionToken() {
    if (!isBosLocalHttpPage())
        return '';
    return readSharedValue(BOS_LOCAL_SESSION_STORAGE_KEY);
}
export function setBosLocalSessionToken(value) {
    if (isBosLocalHttpPage() && value)
        writeSharedValue(BOS_LOCAL_SESSION_STORAGE_KEY, String(value));
    else
...
function postAuthMessage(message) {
    ...
    catch { /* storage sigue sincronizando */ }
}
function emitAuthExpired() {
    if (typeof window === 'undefined' || authExpiryDispatched)
        return;
...
function getTabId() {
    let value = '';
    try {
        value = tabStorage?.getItem(TAB_ID_STORAGE_KEY) || '';
    }
    catch {
        value = '';
    }
    if (!value) {
        ...
        try {
            tabStorage?.setItem(TAB_ID_STORAGE_KEY, value);
        }
        catch { /* memoria no necesita id estable */ }
    }
    return value;
}
...
function writeActiveTabs(tabs) {
    try {
        sharedStorage?.setItem(ACTIVE_TABS_STORAGE_KEY, JSON.stringify(tabs));
    }
    catch { /* sin accion */ }
}
function pruneActiveTabs() {
    ...
    const active = Object.fromEntries(Object.entries(tabs).filter(([, timestamp]) => Number(timestamp) >= now - ACTIVE_TAB_TTL_MS));
...
function cleanupStaleTabHeartbeats() {
...
function authProbeHeaders() {
    ...
    if (localSessionToken)
        headers['X-ARCA-Local-Session'] = localSessionToken;
    if (browserSession)
        headers['X-ARCA-Browser-Session'] = browserSession;
    if (hasRecentHumanActivity())
        headers['X-ARCA-User-Activity'] = '1';
    return headers;
}
function confirmSessionStillValid() {
