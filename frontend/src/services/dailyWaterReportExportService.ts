...
function format(value: unknown, suffix = '', decimals = 2): string {
...
function count(value: unknown, total: unknown): string {
...
function reportFileBase(report: DailyWaterReport): string {
...
function table(title: string, headers: TableHeader[], rows: TableRow[]): string {
  if (!rows.length) {
...
export function exportDailyWaterReportHtml(report: DailyWaterReport, logoUrl?: string): void {
  downloadBlob(
    buildDailyWaterReportHtml(report, logoUrl),
    `${reportFileBase(report)}.html`,
